...
char ssd1306_WriteChar(char ch, FONT_INFO Font, SSD1306_COLOR color) {
    ...
    for (IdxY = 0; IdxY < Font.height; IdxY++) {
        b = Font.char_bitmap[char_Bitmap_Idx + IdxY];
        for (IdxX = 0; IdxX < Font.char_info[char_Info_Idx].width; IdxX++) {
            if ((b << IdxX) & 0x8000) {
                ssd1306_DrawPixel(SSD1306.CurrentX + IdxX, (SSD1306.CurrentY + IdxY), (SSD1306_COLOR) color);
            }
            else {
                ssd1306_DrawPixel(SSD1306.CurrentX + IdxX, (SSD1306.CurrentY + IdxY), (SSD1306_COLOR)!color);
            }
        ...
        if (SSD1306_WIDTH < (SSD1306.CurrentX + Font.char_info[char_Info_Idx].width))
        {
            ssd1306_DrawPixel(SSD1306.CurrentX + IdxX, (SSD1306.CurrentY + Font.char_info[char_Info_Idx].width + 1), (SSD1306_COLOR)!color);
        }
    }
