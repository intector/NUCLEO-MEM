...
UINT        temp_realm_length = 0;
                                        ...
                                        sizeof(server_ptr -> nx_web_http_server_request_resource) - 1))
    {
    	...
    	// checking the URL for empty file name
	char* search_str = "/";
	char* url_str = server_ptr->nx_web_http_server_request_resource;
	
	if (strcmp(search_str, url_str) == 0)
	{
		strcpy(server_ptr->nx_web_http_server_request_resource, MX_WEB_HTTP_SERVER_DEFAULT_HOME_PAGE_NAME);
		resource_length = sizeof(MX_WEB_HTTP_SERVER_DEFAULT_HOME_PAGE_NAME) - 1;
	}
